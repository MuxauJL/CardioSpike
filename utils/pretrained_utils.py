...
def load_model(model, pretrained_checkpoint_path):
    ...
    model.load_state_dict(load_diff_pretrained(model.state_dict(), pretrained_model['state_dict'], 'net'))

def load_diff_pretrained(new_state_dict, pretrained_state_dict, model_name):

    for name, param in new_state_dict.items():
        if name in pretrained_state_dict and model_name in name:
